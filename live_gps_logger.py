...
FIELDS = [
    "session_id", "timestamp_ms", "timestamp_iso", "latitude", "longitude", "accuracy_m", "speed_mps", "bearing_deg",
    "altitude_m", "provider", "raw_provider", "source", "reused",
...
def walk_eta_seconds(distance_m): return int(distance_m / max(WALK_SPEED_MPS, 0.1))

def risk_from_history(station_id, bikes_now):
    h = BIKES_HISTORY[station_id]
    if bikes_now is None: bikes_now = 0
    h.append(int(bikes_now))
    if bikes_now <= LOW_BIKES_THRES: return "low"
    if len(h) >= FALL_WINDOW and all(h[i] > h[i+1] for i in range(len(h)-1)): return "falling"

    return "ok"

def alternative_recommendations(lat, lon):
    with BIKES_LOCK:
        stations = list(LATEST_STATIONS)
    results = []

    for s in stations: 
        lat2, lon2 = s.get("lat"), s.get("lon")
        ...
        results.append({
            "stationid": sid, "name": s.get("name"),
            "distance_m": int(d), "walk_eta_s": int(eta_s), 
            "available_bikes": bikes, "available_stands": stands, "score": score
        })
    results.sort(key=lambda x: x["score"], reverse=True)
    return results[:ALT_TOP_K]
...
def post_latest_summary(session_id, summary):
    if not FIREBASE_DB_URL: return
    try: requests.post(fb_url(f"/analytics/latest/{session_id}"), json=summary, timeout=5)
    except Exception as e: print("Watn latest summary post failed", e, file=sys.stderr)


def post_recommendations(session_id, recs):
    if not FIREBASE_DB_URL: return
    doc = {"timestampms": int(time.time()*1000), "items": recs}
    try: requests.post(fb_url(f"/analytics/recommendations/{session_id}"), json=doc, timeout=6)
    except Exception as e: print("Warn recommendations post failed", e, file=sys.stderr) 
    ...
    try:
        while True:
            ...
            if loc:
                ...
                # -------------------------------------------------
                # Enrich with nearest station, risk flag, and recs
                # -------------------------------------------------
                lat = row["latitude"]
                lon = row["longitude"]
                if lat is not None and lon is not None:
                    ...
                    if best:
                        # bikes/stands currently at nearest station
                        bikes_now = int(best.get("available_bikes", 0) or 0)
                        stands_now = int(best.get("available_stands", 0) or 0)
                        eta_s = walk_eta_seconds(d)

                        # derive a robust station id
                        sid = best.get("station_id") or best.get("stationid") or best.get("number") or best.get("name")

                        # risk flag from rolling history
                        risk = risk_from_history(sid, bikes_now)

                        # enrich row
                        row["nearest_station_id"] = sid
                        row["nearest_station_name"] = best.get("name")
                        row["nearest_dist_m"] = int(d)
                        row["nearest_walk_eta_s"] = int(eta_s)
                        row["nearest_bikes"] = bikes_now
                        row["nearest_stands"] = stands_now
                        row["risk_flag"] = risk

                        # print nearest station info to console
                        print(
                            f"Nearest station: {best.get('name')} at {int(d)} m | "
                            f"bikes={bikes_now} stands={stands_now} | risk={risk}"
                        )

                        # latest summary for dashboard (Task 1 + 2)
                        latest = {
                            "timestamp_ms": row["timestamp_ms"],
                            "lat": row["latitude"],
                            "lon": row["longitude"],
                            "nearest_station_id": sid,
                            "nearest_station_name": best.get("name"),
                            "nearest_dist_m": int(d),
                            "nearest_walk_eta_s": int(eta_s),
                            "nearest_bikes": bikes_now,
                            "nearest_stands": stands_now,
                            "risk_flag": risk,
                        }
                        post_latest_summary(SESSION, latest)

                        # alternative station recommendations (Task 3)
                        recs = alternative_recommendations(lat, lon)
                        post_recommendations(SESSION, recs)

                # write to CSV and Firebase (after enrichment)
                w.writerow(row)
                f.flush()
